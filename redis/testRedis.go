...
func stringOperation( client *redis.Client ) {
	...
	fmt.Println( "age的值：", val ) // age 的值为21
...
func setOperation( client *redis.Client ) {
	...
	fmt.Println( "Is Bush in blacklist的值： ", isMember )
	...
	// 获取到的元素是 "the Elder"
	fmt.Println( "Inter result的值： ", names )
...
func hashOperation( client *redis.Client ) {
	...
	fmt.Println( "fields in user_test的值：", fields )
	...
	fmt.Println( "field count in user_xys的值： ", length ) // 字段个数为2
